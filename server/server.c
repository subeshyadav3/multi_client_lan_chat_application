...
/* ── room message history (so late-joining users see recent messages) ── */
#define ROOM_HISTORY_MAX 50

typedef struct RoomMessage {
    char *text;  /* full protocol line, e.g. "PUBLIC|room|user|msg|ts" */
        ...
        for (ssize_t i = 0; i < n; i++) {
            if (buf[i] == '\n') {
                ...
                if (strcmp(cmd, "LOGIN") == 0 && parts >= 3) {
                    ...
                    prev_room[sizeof(prev_room)-1] = '\0';
                    strncpy(c->current_room, "general", MAX_ROOM_NAME-1);
                    c->current_room[MAX_ROOM_NAME-1] = '\0';
                    char msg[256];
