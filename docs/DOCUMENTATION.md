...
2. Server verifies the size (≤ 64 MiB limit). If upload slots (max 2) or global budget (1 MiB) are full, the request is queued in a FIFO queue (cap 16) with `FILE_WAIT`.
